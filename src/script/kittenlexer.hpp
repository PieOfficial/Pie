...
class KittenLexer {
 ...
 public:
  inline bool is_stringq(char c) const { return stringqs.count(c) > 0; }
  ...
  inline bool is_ignore(char c) const { return ignores.count(c) > 0; }
  inline bool is_newline(char c) const { return as_newline.count(c) > 0; }
  inline bool is_extract(char c) const { return extracts.count(c) > 0; }
  inline bool is_lineskip(char c) const { return lineskips.count(c) > 0; }
  ...
  inline KittenLexer& add_stringq(char c) {
    if (!is_stringq(c)) stringqs[c] = true;
    return *this;
  }
  inline KittenLexer& add_ignore(char c) {
    if (!is_ignore(c)) ignores[c] = true;
    return *this;
  }
  inline KittenLexer& add_extract(char c) {
    if (!is_extract(c)) extracts[c] = true;
    return *this;
  }
  inline KittenLexer& add_lineskip(char c) {
    if (!is_lineskip(c)) lineskips[c] = true;
    return *this;
  }
  ...
  inline KittenLexer& add_linebreak(char c) {
    if (!is_newline(c)) as_newline[c] = true;
    return *this;
  }
  ...
  inline lexed_kittens lex(std::string src) {
    ...
    for (size_t i = 0; i < src.size(); ++i) {
      if (stringqs.empty() && opens.empty() &&
          (is_ignore(src[i]) || is_newline(src[i]) ||
           match_con_any(con_newlines, src[i]) ||
           match_con_any(con_ignores, src[i]))) {
        if (is_newline(src[i]) || match_con_any(con_newlines, src[i]))
          suntil_newline = false;
        if ((is_ignore(src[i]) || match_con_any(con_ignores, src[i])) &&
            suntil_newline)
          continue;
        if (tmp != "" || !erase_emptys) {
      ...
      } else if (stringqs.empty() && is_capsule_open(src[i]) &&
                 !suntil_newline) {
        ...
        opens.push(src[i]);
        tmp += src[i];
      } else if (stringqs.empty() && is_capsule_close(src[i]) &&
                 !suntil_newline) {
        ...
        if (opens.empty()) {
          ...
          tmp = "";
                 ...
                 (is_stringq(src[i]) || match_con_any(con_stringqs, src[i])) &&
                 !suntil_newline) {
        if (stringqs.empty()) {
          if (tmp != "" || !erase_emptys) {
            ret.push_back(KittenToken{tmp, false, line});
      ...
      } else if (opens.empty() && stringqs.empty() &&
                 (is_extract(src[i]) || match_con_any(con_extracts, src[i])) &&
                 !suntil_newline) {
        if (tmp != "" || !erase_emptys)
          ret.push_back(KittenToken{tmp, false, line});
        tmp = src[i];
        ret.push_back(KittenToken{tmp, false, line});
        tmp = "";
      } else if (stringqs.empty() && (is_lineskip(src[i]) ||
                                      match_con_any(con_lineskips, src[i]))) {
        suntil_newline = true;
      } else if ((is_newline(src[i]) || match_con_any(con_newlines, src[i])) &&
                 stringqs.empty() && suntil_newline) {
        suntil_newline = false;
      } else if (opens.empty() && /*stringqs.empty() &&*/ src[i] == '\\') {
        if (i + 1 != src.size()) {
          char n = src[i + 1];
          if (backslash_opts.count(n) != 0)
            tmp += backslash_opts[n];
          else
            tmp += "\\" + std::string(1, n);
          ++i;
        } else {

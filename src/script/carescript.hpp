...
struct ScriptOperator {
  ...
  // 2 => both
  int unary = 0;
  ScriptVariable (*run)(ScriptVariable left, ScriptVariable right,
                        ScriptSettings& settings);

  ScriptVariable (*run_unary)(ScriptVariable left, ScriptVariable right,
                              ScriptSettings& settings);
};
...
std::string run_script(std::string label_name,
                       std::map<std::string, ScriptLabel> label,
                       std::filesystem::path parent_path = "",
                       std::vector<ScriptVariable> args = {});
std::map<std::string, ScriptLabel> into_labels(std::string source);
std::vector<ScriptVariable> parse_argumentlist(std::string source,
                                               ScriptSettings& settings);
ScriptVariable evaluate_expression(std::string source,
                                   ScriptSettings& settings);

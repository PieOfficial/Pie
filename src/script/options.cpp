...
void print_entry(IniDictionary current) {
  std::string repo =
      (std::string)current["author"] + "/" + (std::string)current["name"];
...
bool browse(std::string file) {
  ...
  while (!exit) {
    ...
    std::string repo = (std::string)entries[current]["author"] + "/" +
                       (std::string)entries[current]["name"];
    repo = to_lowercase(repo);

    std::cout << ">>> " << entries[current]["name"] << " (" << (current + 1)
              << " of " << entries.size() << ") <<<\n";
    ...
    std::string inp;
    std::getline(std::cin, inp);
    if ((inp == "n" || inp == "N" || inp == "next") &&
        current != entries.size() - 1) {
      ++current;
    } else if ((inp == "p" || inp == "P" || inp == "previous") &&
               current != 0) {
      --current;
      ...
      cat_sleep(1000);
    } else if ((inp == "u" || inp == "U" || inp == "uninstall") &&
               installed(repo)) {

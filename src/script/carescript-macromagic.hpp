...
#define _cc_error(...) \
do { \
    settings.error_msg = __VA_ARGS__; \
    return carescript::script_null; \
} while(0)

#define _cc_error_if(expr, ...) \
if(!!(expr)) \
    _cc_error(__VA_ARGS__); \
else \
    do {} while(0)
...
#define _cc_defer(...) \
    __VA_ARGS__ _cc_empty()

#define _cc_obstruct(...) \
    __VA_ARGS__ _cc_defer(_cc_empty)()

#define _cc_eval(...)  \
    _cc_eval1(_cc_eval1(_cc_eval1(__VA_ARGS__)))

#define _cc_eval1(...) \
    _cc_eval2(_cc_eval2(_cc_eval2(__VA_ARGS__)))

#define _cc_eval2(...) \
    _cc_eval3(_cc_eval3(_cc_eval3(__VA_ARGS__)))

#define _cc_eval3(...) \
    _cc_eval4(_cc_eval4(_cc_eval4(__VA_ARGS__)))

#define _cc_eval4(...) \
    _cc_eval5(_cc_eval5(_cc_eval5(__VA_ARGS__)))

#define _cc_eval5(...) \
    __VA_ARGS__

#define _cc_second(a, b, ...) \
    b
...
#define cc_operator_same_type(variable1, variable2, op) \
    if((variable1).get_type() != (variable2).get_type()) { \
        _cc_error(#op ": " #variable1 " and "#variable2 " must have the same type (" #variable1 ": " + (variable1).get_type() + " | " #variable2 ": " + (variable2).get_type() + ")");\
    } else do {} while (0)

#define _cc_requires1(variable, type1, ...) \
    _cc_second(__VA_OPT__(,) _cc_requires2(variable, type1, __VA_ARGS__), _cc_requires3(variable, type1))

#define _cc_requires2(variable, type1, ...) \
    _cc_requires3(variable, type1) && _cc_obstruct(_cc_requires4)()(variable, __VA_ARGS__)

#define _cc_requires3(variable, type) \
    !is_typeof<type>(variable)

#define _cc_requires4() \
    _cc_requires1

#define _cc_chain(...) \
    _cc_eval(_cc_chain1(__VA_ARGS__))

#define _cc_chain1(...) \
    _cc_second(__VA_OPT__(,) _cc_chain2(__VA_ARGS__), )

#define _cc_chain2(car, ...) \
    + (car()).get_type() + " " _cc_obstruct(_cc_chain3)()(__VA_ARGS__)
